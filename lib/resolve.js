...
class Resolver {
  constructor(dbPath) {
    ...
    for (const l of this.lines) {
      const cur = this.shabadStartAng.get(l.shabad_id)
      if (cur === undefined || l.ang < cur) this.shabadStartAng.set(l.shabad_id, l.ang)
    }
  ...
  resolve(heading, bani, sourceId) {
    ...
    const ok = coverage >= THRESHOLDS.minCoverage
      ...
      && (avgMargin >= THRESHOLDS.minMargin || counterAgrees)
      && !angImpossible
